# leetcode

| # | tooltip |
| --- | --- |
| 1 | findIndex |
| 2 | stack |
| 7 | reverse |
| 8 | code |
| 9 | ! |
| 10 | RegExp |
| 12 | ! |
| 13 | 递归 |
| 18 | code |
| 20 | stack |
| 21 | ! |
| 24 | ! |
| 26 | ! |
| 27 | ! |
| 28 | indexOf |
| 29 | / + restrict |
| 33 | findIndex |
| 35 | findIndex |
| 41 | toCount |
| 46 | code |
| 48 | ! |
| 50 | ** |
| 54 | code |
| 56 | _.sort + two pointers |
| 59 | ! vectors |
| 65 | isNaN |
| 66 | ! |
| 69 | Math |
| 70 | dp |
| 73 | ?，应该可以用 bitWise 实现原地，懒得写 |
| 78 | ! |
| 80 | 双指针 |
| 81 | find |
| 88 | splice + sort |
| 89 | 递归，末尾的 pattern 是 0110 |
| 90 | ! + _.uniqBy + JSON.stringify |
| 91 | code |
| 94 | tree |
| 98 | tree |
| 100 | tree |
| 101 | tree |
| 102 | tree |
| 103 | tree |
| 104 | tree |
| 105 | tree |
| 106 | tree |
| 107 | tree |
| 108 | tree |
| 111 | - |
| 112 | tree |
| 113 | tree |
| 114 | tree |
| 118 | ! |
| 119 | 181 + 打表 |
| 121 | code |
| 125 | reverse |
| 129 | tree |
| 136 | toCount |
| 137 | toCount |
| 141 | 搜索链表有环 |
| 144 | tree |
| 145 | tree |
| 153 | sort |
| 154 | sort |
| 155 | Array + reduce |
| 162 | 二分 |
| 167 | 两端逼近 |
| 168 | ! |
| 169 | toCount |
| 171 | ! |
| 172 | 含有因数 5 的数量 |
| 173 | tree |
| 175 | sql |
| 176 | sql |
| 177 | sql |
| 180 | sql |
| 181 | sql |
| 182 | sql |
| 183 | sql |
| 184 | sql |
| 190 | toString + padStart + reverse + parseInt |
| 191 | toString(2) |
| 196 | sql |
| 197 | sql |
| 198 | dp |
| 202 | 记录 visited |
| 203 | ! |
| 204 | sqrt |
| 205 | 双向 dict |
| 206 | ! |
| 215 | sort |
| 217 | toCount |
| 225 | ! |
| 226 | tree |
| 229 | toCount |
| 230 | tree |
| 231 | toString(2) |
| 232 | ! |
| 234 | toArray |
| 237 | ! |
| 257 | tree |
| 258 | (num - 1) % 9 + 1 |
| 260 | toCount |
| 263 | ! |
| 264 | ! |
| 268 | toCount |
| 273 | code |
| 278 | 二分法 |
| 283 | 双指针 |
| 287 | toCount |
| 289 | bitWise |
| 290 | 双向 dict |
| 292 | Boolean(n % 4) |
| 315 | _.sortedIndex |
| 319 | sqrt 完全平方数的个数 |
| 326 | 打表 |
| 338 | 每 1, 2, 4, 8, 16, ... 重复，并在原有的数字上 + 1 |
| 341 | ! |
| 342 | 打表 |
| 344 | reverse |
| 345 | ! |
| 347 | toCount |
| 349 | toCount |
| 350 | toCount |
| 357 | 打表，9位数及以上可以不考虑 |
| 367 | isSquare |
| 371 | ! |
| 374 | 二分法 |
| 380 | Object |
| 381 | Object + 级数 + find |
| 387 | toCount |
| 389 | toCount |
| 404 | tree |
| 405 | + 0xffffffff + 1 toString |
| 406 | code |
| 412 | ! |
| 414 | ? |
| 419 | createUnionFindSet |
| 432 | _.maxBy |
| 441 | sqrt 要么是 sqrt 要么是 sqrt - 1 |
| 442 | toCount |
| 448 | toCount |
| 445 | toArray |
| 449 | ! |
| 451 | toCount + countToFrequency + sort |
| 461 | ^ + toCount |
| 476 | log 得到位数 + ^ |
| 493 | _.sortedIndex |
| 495 | ! |
| 496 | stack |
| 500 | replace new RegExp(, 'g') |
| 501 | tree |
| 504 | toString(7) |
| 507 | 搜索完美数并打表 |
| 509 | dp |
| 520 | ! |
| 520 | code |
| 525 | 积分 |
| 532 | toCount |
| 535 | ! |
| 537 | ! |
| 538 | tree |
| 541 | reverse |
| 543 | tree |
| 551 | ! |
| 557 | reverse |
| 561 | 贪心 |
| 566 | ! |
| 575 | toCount |
| 595 | sql |
| 596 | sql |
| 599 | ! |
| 605 | ! |
| 617 | tree |
| 620 | sql |
| 627 | sql |
| 628 | sort 3正/1正2负，没有负数的情况并不例外 |
| 633 | - ** 2 + isSquare |
| 637 | tree |
| 639 | code dp |
| 645 | toCount |
| 649 | 队列 |
| 650 | 递归，因数的和 |
| 654 | tree |
| 657 | toCount |
| 659 | toCount + ! |
| 661 | ! |
| 674 | ! |
| 680 | reverse + ! |
| 682 | stack |
| 684 | createUnionFindSet |
| 692 | toCount + countToFrequency + sort |
| 693 | toString(2) + shouldBe = i % 2 === 0 ? '1' : '0' |
| 696 | code |
| 700 | - |
| 703 | ! |
| 704 | findIndex |
| 709 | ! |
| 717 | 遇到 1 就 step = 2 |
| 728 | ! |
| 739 | code |
| 744 | sort + find |
| 746 | 动态规划 |
| 747 | ! |
| 763 | two pointers |
| 766 | ! |
| 771 | ! |
| 775 | 充要：每一项介于 i - 1 和 i + 1 之间 |
| 784 | ! |
| 791 | ! |
| 793 | 关键数字在 6, 31, 156, ... keyNumber = 5 * keyNumber + 1 |
| 804 | ! |
| 806 | ! |
| 807 | ! |
| 814 | tree |
| 821 | ! |
| 825 | sort + 解析几何 |
| 832 | ! |
| 844 | stack + isEqual |
| 852 | 下一个数不再递增时 |
| 858 | gcd |
| 859 | stack + toCount + find |
| 860 | ! |
| 867 | ! |
| 868 | toString(2) |
| 872 | tree |
| 877 | 先手必胜 |
| 883 | ! |
| 888 | sort + 双指针 |
| 890 | ! |
| 896 | ! |
| 905 | ! |
| 908 | ! |
| 922 | ! |
| 926 | ! 当 1 的个数少于紧跟的 0 的个数时可以贪心 |
| 929 | ! |
| 933 | ! |
| 938 | tree |
| 942 | sort |
| 944 | sort |
| 950 | 评论区 |
| 954 | toCount + 贪心 + 正负 |
| 959 | createUnionFindSet |
| 969 | sort + findIndex NlogN? |
| 977 | sort |
| 999 | findIndex + ! |
| 1010 | ! |
| 1021 | ! |
| 1025 | 找规律 |
| 1051 | sort |
| 1078 | filter |
| 1108 | ! |
| 1114 | Semaphore |
| 1115 | Semaphore |
| 1137 | dp |
| 1162 | 四方向暴力 |
| 1207 | toCount |
| 1221 | ! |
| 1252 | (n - rowFlip) * colFlip + rowFlip * (m - colFlip) |
| 1266 | ! |
| 1281 | toArray |
| 1290 | ! |
| 1295 | Math.log10 |
| 1299 | ! |
| 1313 | _.chunk + Array(a).fill(b) |
| 1342 | ! |
| 1351 | _.flatten |
| 1365 | toCount + reduce |
| 1379 | - |
| 1389 | splice |
| 1395 | - |
| 1431 | - |
| 1436 | _.difference |
| 1450 | filter |
| 1464 | sort |
| 1470 | _.flatten _.unzip |
| 1476 | - |
| 1480 | - |
| 1486 | ^= |
| 1502 | ! |
| 1512 | - |
| 1551 | - |
| 1689 | |
| 1796 | 找规律 |
| LCP 01 | _.zip ? |
| LCP 06 | Math.ceil |
| 剑指 05 | replace(/ /g, '%20') |
| 剑指 06 | reverse |
| 剑指 17 | Array.from |
| 剑指 22 | 双指针 |
| 剑指 27 | ! |
| 剑指 55 - I | ! |
| 剑指 58 - II | slice |
| 剑指 64 | - |
| 面试 02.02 | 双指针 |
| 面试 02.03 | ! |
| 面试 04.02 | 二分 |
| 面试 16.01 | - |

## footnote

1. `!` 代表暴力方法
1. `?` 代表用了过于复杂的方法
1. `code` 代表列出了题解，题解可能比较复杂
1. `-` 代表在 `index.js` 中列出了题解，题解一般简单
1. 函数名的提示要么是 js 自带方法，要么在 polyfill 里
1. leetcode 支持 lodash
1. 无视了题目中的时间复杂性和禁用某些库的要求
